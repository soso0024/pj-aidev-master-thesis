...
class TestRunner:
    ...
    def run_pytest(self, test_file_path: str) -> TestResult:
        ...
        # Create a directory for temporary coverage files
        coverage_dir = Path.cwd() / ".coverage_tmp"
        coverage_dir.mkdir(exist_ok=True)

        # Generate unique coverage file name to avoid conflicts in parallel execution
        coverage_file = coverage_dir / f"coverage_{uuid.uuid4().hex}.json"
        ...
        try:
            ...
            c0_coverage, c1_coverage = self._parse_coverage_from_json(coverage_file)
            ...
            # Clean up the coverage file after use
            if coverage_file.exists():
                try:
                    coverage_file.unlink()
                except Exception:
                    pass  # Ignore cleanup errors (file may be locked on Windows)

    def _parse_coverage_from_json(self, coverage_file: Path) -> tuple[float, float]:
        """
