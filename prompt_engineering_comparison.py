...
class PromptEngineeringAnalyzer:
    ...
    def create_comparison_plots(self, metrics: Dict[str, Dict[str, float]], output_dir: str = '.') -> None:
        """
        比較グラフを作成
        ...
        bars = ax.bar(config_labels, success_rates, color=colors, alpha=0.8, edgecolor='black')
        ax.set_ylabel('Success Rate (%)', fontsize=12, fontweight='bold')
        ax.set_xlabel('Prompt Engineering Method', fontsize=12, fontweight='bold')
        ax.set_title('Success Rate Comparison by Prompt Engineering Method', 
        ...
        # 2. Coverage比較グラフ
        fig, ax = plt.subplots(figsize=(10, 6))
        bars = ax.bar(config_labels, coverages, color=colors, alpha=0.8, edgecolor='black')
        ax.set_ylabel('Average Code Coverage (%)', fontsize=12, fontweight='bold')
        ax.set_xlabel('Prompt Engineering Method', fontsize=12, fontweight='bold')
        ax.set_title('Code Coverage Comparison by Prompt Engineering Method', 
                     fontsize=14, fontweight='bold', pad=20)
        ax.set_ylim(0, 100)
        ax.grid(axis='y', alpha=0.3, linestyle='--')
        ...
        # Success Rate
        bars1 = ax1.bar(config_labels, success_rates, color=colors, alpha=0.8, edgecolor='black')
        ...
        # Coverage
        bars2 = ax2.bar(config_labels, coverages, color=colors, alpha=0.8, edgecolor='black')
        ...
        fig.suptitle('Prompt Engineering Methods Comparison: Success Rate and Coverage', 
                     fontsize=15, fontweight='bold', y=0.98)
        plt.tight_layout()
        plt.savefig(output_path / 'combined_comparison.png', dpi=300, bbox_inches='tight')
        ...
        ax.plot(angles, values_sr, 'o-', linewidth=2, label='Success Rate', color='#3498db')
        ax.fill(angles, values_sr, alpha=0.25, color='#3498db')
        
        # Coverageをプロット
        values_cov = coverages + [coverages[0]]
        ax.plot(angles, values_cov, 'o-', linewidth=2, label='Coverage', color='#e74c3c')
        ...
        ax.set_title('Radar Chart: Success Rate vs Coverage\nby Prompt Engineering Method', 
        ...
        for i, (config, label, color) in enumerate(zip(configs, config_labels, colors)):
            sr = success_rates[i]
            cov = coverages[i]
            ax.scatter(sr, cov, s=500, alpha=0.7, color=color, edgecolors='black', linewidth=2)
            ax.text(sr, cov, label, ha='center', va='center', fontsize=10, fontweight='bold')
        
        ax.set_xlabel('Success Rate (%)', fontsize=12, fontweight='bold')
        ax.set_ylabel('Average Code Coverage (%)', fontsize=12, fontweight='bold')
        ax.set_title('Success Rate vs Code Coverage\nby Prompt Engineering Method', 
                     fontsize=14, fontweight='bold', pad=20)
        ax.set_xlim(0, 105)
        ax.set_ylim(0, 105)
...
def main():
    ...
    # 除外モデルを除いた統合用のアナライザー
    excluded_models = ['generated_tests_claude-3-haiku', 'generated_tests_claude-opus-4-1']
    ...
    if combined_analyzer.data:
        ...
        try:
            ...
            # 統合グラフの作成
            combined_analyzer.create_comparison_plots(combined_metrics, str(output_dir_combined))
    ...
    if filtered_analyzer.data:
        ...
        try:
            ...
            # フィルター済みグラフの作成
            filtered_analyzer.create_comparison_plots(filtered_metrics, str(output_dir_filtered))
