...
class CrossModelPlots:
    ...
    def __init__(self, model_data: Dict[str, List[Dict[str, Any]]], config_order: List[str]):
        """
        ...
        if len(self.model_names) <= len(base_colors):
            ...
            self.colors = (base_colors * ((len(self.model_names) // len(base_colors)) + 1))[:len(self.model_names)]
    ...
    def plot_bug_detection_rate_comparison(self, output_dir: Path) -> None:
        ...
        ax.set_ylabel("Bug Detection Rate (%)" if self.is_humanevalpack else "Success Rate (%)", 
                      fontweight="bold", fontsize=16)
        title = "Bug Detection Rate Comparison Across Models" if self.is_humanevalpack else "Test Success Rate Comparison Across Models"
        ...
        ax.legend(loc="center left", bbox_to_anchor=(1, 0.5), framealpha=0.95, fontsize=13, ncol=1)
        ax.grid(True, alpha=0.3, axis="y", linewidth=0.8)
        ax.set_ylim(0, 105)

        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_1a_bug_detection_rate.png", dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def plot_test_generation_success_rate(self, output_dir: Path) -> None:
        ...
        for i, model_name in enumerate(self.model_names):
            ...
            for config in self.config_order:
                ...
                if config_data:
                    ...
                    success_count = sum(1 for d in config_data if d.get('evaluation_success', False))
                    success_rate = (success_count / len(config_data)) * 100
                    success_rates.append(success_rate)
                else:
                    success_rates.append(0)
        ...
        ax.set_xlabel("プロンプトタイプ", fontweight="bold", fontsize=16)
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_1b_test_generation_success_rate.png", dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def plot_cost_comparison(self, output_dir: Path) -> None:
        ...
        ax1.set_xticklabels([self._clean_model_name(m) for m in self.model_names], rotation=45, ha="right", fontsize=12)
        ...
        for i, model_name in enumerate(self.model_names):
            ...
            for config in self.config_order:
                config_data = [d for d in data if d.get("config_type") == config]
                costs = [d.get("total_cost", 0) for d in config_data if d.get("total_cost", 0) > 0]
        ...
        ax2.set_title(
            "Average Cost by Configuration", fontweight="bold", fontsize=15
        )
        ax2.set_xticks(x)
        ax2.set_xticklabels(self.config_order, fontsize=12)
        ax2.tick_params(axis='y', labelsize=12)
        # Place legend outside the plot area (right side)
        ax2.legend(loc="center left", bbox_to_anchor=(1, 0.5), framealpha=0.95, fontsize=11)
        ax2.grid(True, alpha=0.3, axis="y", linewidth=0.8)

        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_2_cost_comparison.png", dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def plot_coverage_comparison(self, output_dir: Path) -> None:
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_3a_c0_coverage.png", dpi=300, bbox_inches="tight")
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_3b_c1_coverage.png", dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def plot_efficiency_comparison(self, output_dir: Path) -> None:
        ...
        ax.set_ylabel("Efficiency Score (Success Rate % / $)", fontweight="bold", fontsize=16)
        ax.set_title(
        ...
        ax.set_xticklabels([self._clean_model_name(m) for m in self.model_names], rotation=45, ha="right", fontsize=13)
        ax.tick_params(axis='y', labelsize=14)
        ax.grid(True, alpha=0.3, axis="y", linewidth=0.8)
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_4_efficiency.png", dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def plot_fix_attempts_comparison(self, output_dir: Path) -> None:
        ...
        ax.tick_params(axis='y', labelsize=14)
        # Place legend outside the plot area (right side)
        ax.legend(loc="center left", bbox_to_anchor=(1, 0.5), framealpha=0.95, fontsize=13)
        ax.grid(True, alpha=0.3, axis="y", linewidth=0.8)

        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_5_fix_attempts.png", dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def plot_token_usage_comparison(self, output_dir: Path) -> None:
        ...
        for i, model_name in enumerate(self.model_names):
            ...
            for config in self.config_order:
                config_data = [d for d in data if d.get("config_type") == config]
                tokens = [d.get("total_input_tokens", 0) for d in config_data if d.get("total_input_tokens", 0) > 0]
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_6a_input_tokens.png", dpi=300, bbox_inches="tight")
        plt.close()
        ...
        for i, model_name in enumerate(self.model_names):
            ...
            for config in self.config_order:
                config_data = [d for d in data if d.get("config_type") == config]
                tokens = [d.get("total_output_tokens", 0) for d in config_data if d.get("total_output_tokens", 0) > 0]
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_6b_output_tokens.png", dpi=300, bbox_inches="tight")
        plt.close()
        ...
        for i, model_name in enumerate(self.model_names):
            ...
            for config in self.config_order:
                config_data = [d for d in data if d.get("config_type") == config]
                tokens = [d.get("total_tokens", 0) for d in config_data if d.get("total_tokens", 0) > 0]
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_6c_total_tokens.png", dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def plot_cost_efficiency_scatter(self, output_dir: Path) -> None:
        ...
        for i, model_name in enumerate(self.model_names):
            ...
            for config in self.config_order:
                ...
                ax.scatter(
                    ...
                    color=self.colors[i],
                    marker=marker,
                    label=f"{self._clean_model_name(model_name)} - {config}" if config == self.config_order[0] else "",
                    edgecolors='#333333',
                    linewidth=1.2,
                )
        ...
        ax.set_ylabel("Bug Detection Rate (%)" if self.is_humanevalpack else "Success Rate (%)", 
                      fontweight="bold", fontsize=16)
        title = "Cost vs Bug Detection Rate Trade-off" if self.is_humanevalpack else "Cost vs Success Rate Trade-off"
        ax.set_title(
        ...
        # Place legend outside the plot area (right side)
        ax.legend(loc="center left", bbox_to_anchor=(1, 0.5), framealpha=0.95, fontsize=11)

        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_7_cost_vs_success.png", dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def plot_model_config_heatmap(self, output_dir: Path) -> None:
        ...
        colors_map = ['#FF4B00', '#F6AA00', '#03AF7A']  # Red -> Yellow -> Green from our palette
        n_bins = 100
        ...
        title_text = "Bug Detection Rate Heatmap: Models vs Configurations" if self.is_humanevalpack else "Success Rate Heatmap: Models vs Configurations"
        ax.set_title(title_text, fontweight="bold", fontsize=18, pad=20)
        ax.tick_params(axis='both', labelsize=13)
        cbar = fig.colorbar(im, ax=ax)
        cbar.set_label("Bug Detection Rate (%)" if self.is_humanevalpack else "Success Rate (%)", 
                       fontsize=14, fontweight="bold")
        cbar.ax.tick_params(labelsize=12)

        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_8_heatmap.png", dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def plot_overall_rankings(self, output_dir: Path) -> None:
        ...
        # Plot 1: Success Rate
        sorted_models = sorted(self.model_names, key=lambda m: metrics[m]["success_rate"], reverse=True)
        colors_sorted = [self.colors[self.model_names.index(m)] for m in sorted_models]
        ax1.barh(range(len(sorted_models)), [metrics[m]["success_rate"] for m in sorted_models], 
                 color=colors_sorted, alpha=0.85, edgecolor='#333333', linewidth=1.2)
        ax1.set_yticks(range(len(sorted_models)))
        ax1.set_yticklabels([self._clean_model_name(m) for m in sorted_models], fontsize=12)
        label_text = "Bug Detection Rate (%)" if self.is_humanevalpack else "Success Rate (%)"
        ax1.set_xlabel(label_text, fontweight="bold", fontsize=13)
        title_text = "Ranking by Bug Detection Rate" if self.is_humanevalpack else "Ranking by Success Rate"
        ax1.set_title(title_text, fontweight="bold", fontsize=14)
        ax1.tick_params(axis='x', labelsize=12)
        ax1.grid(True, alpha=0.3, axis="x", linewidth=0.8)
        for i, model in enumerate(sorted_models):
            ax1.text(metrics[model]["success_rate"], i, f" {metrics[model]['success_rate']:.1f}%", 
                    va="center", fontsize=11, fontweight="bold")

        # Plot 2: Cost (lower is better)
        sorted_models = sorted(self.model_names, key=lambda m: metrics[m]["avg_cost"])
        colors_sorted = [self.colors[self.model_names.index(m)] for m in sorted_models]
        ax2.barh(range(len(sorted_models)), [metrics[m]["avg_cost"] for m in sorted_models], 
                 color=colors_sorted, alpha=0.85, edgecolor='#333333', linewidth=1.2)
        ax2.set_yticks(range(len(sorted_models)))
        ax2.set_yticklabels([self._clean_model_name(m) for m in sorted_models], fontsize=12)
        ax2.set_xlabel("Average Cost ($)", fontweight="bold", fontsize=13)
        ax2.set_title("Ranking by Cost (Lower is Better)", fontweight="bold", fontsize=14)
        ax2.tick_params(axis='x', labelsize=12)
        ax2.grid(True, alpha=0.3, axis="x", linewidth=0.8)
        for i, model in enumerate(sorted_models):
            ax2.text(metrics[model]["avg_cost"], i, f" ${metrics[model]['avg_cost']:.4f}", 
                    va="center", fontsize=11, fontweight="bold")

        # Plot 3: Coverage
        sorted_models = sorted(self.model_names, key=lambda m: metrics[m]["avg_coverage"], reverse=True)
        colors_sorted = [self.colors[self.model_names.index(m)] for m in sorted_models]
        ax3.barh(range(len(sorted_models)), [metrics[m]["avg_coverage"] for m in sorted_models], 
                 color=colors_sorted, alpha=0.85, edgecolor='#333333', linewidth=1.2)
        ax3.set_yticks(range(len(sorted_models)))
        ax3.set_yticklabels([self._clean_model_name(m) for m in sorted_models], fontsize=12)
        ...
        for i, model in enumerate(sorted_models):
            ax3.text(metrics[model]["avg_coverage"], i, f" {metrics[model]['avg_coverage']:.1f}%", 
                    va="center", fontsize=11, fontweight="bold")

        # Plot 4: Fix Attempts (lower is better)
        sorted_models = sorted(self.model_names, key=lambda m: metrics[m]["avg_attempts"])
        colors_sorted = [self.colors[self.model_names.index(m)] for m in sorted_models]
        ax4.barh(range(len(sorted_models)), [metrics[m]["avg_attempts"] for m in sorted_models], 
                 color=colors_sorted, alpha=0.85, edgecolor='#333333', linewidth=1.2)
        ax4.set_yticks(range(len(sorted_models)))
        ax4.set_yticklabels([self._clean_model_name(m) for m in sorted_models], fontsize=12)
        ax4.set_xlabel("Average Fix Attempts", fontweight="bold", fontsize=13)
        ax4.set_title("Ranking by Fix Attempts (Lower is Better)", fontweight="bold", fontsize=14)
        ax4.tick_params(axis='x', labelsize=12)
        ax4.grid(True, alpha=0.3, axis="x", linewidth=0.8)
        for i, model in enumerate(sorted_models):
            ax4.text(metrics[model]["avg_attempts"], i, f" {metrics[model]['avg_attempts']:.1f}", 
                    va="center", fontsize=11, fontweight="bold")

        plt.suptitle("Overall Model Rankings by Key Metrics", fontweight="bold", fontsize=18, y=0.995)
        plt.tight_layout()
        plt.savefig(output_dir / "cross_model_9_rankings.png", dpi=300, bbox_inches="tight")
        plt.close()
